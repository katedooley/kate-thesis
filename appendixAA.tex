...
\section{Photodiodes}
\label{sec:pds}
The basic photoconductive photodiode is depicted in Figure
\ref{fig:PD}. A negative voltage applied to the anode, called the bias
voltage $V_{\mathrm{bias}}$, does not yield any current across the
photodiode until electrons are released by the energy of photons
striking the diode. Electrons flow in the direction of the cathode
which is connected to ground, and produce a current
\begin{equation}
i_\gamma = \frac{q_e}{h \nu} P \epsilon
\end{equation}
...
\begin{figure}
\begin{centering}
\subfigure{\includegraphics[width=0.9\textwidth]{/Users/kate/Documents/diagrams/photodiode.pdf}}
\caption[Schematic of a basic photoconductive photodiode.]{Schematic of a basic photoconductive photodiode.}
\label{fig:PD}
\end{centering}
\end{figure}

To have an accurate measure of the power on the photodiode, we must
measure the produced current $i_\gamma$. This is accomplished by
inserting a resistor in series between the cathode and ground and
measuring the voltage drop across the resistor, whose resistance is
called the \emph{transimpedance}, $R_t$.
%, $i_\gamma = V / R$. This resistance is called the \emph{transimpedance}.
A typical voltmeter, despite having high resistance, will inevitably
draw some small amount of current, thereby underestimating the current
produced by the light. The clever solution is to make use of an op-amp
as a voltmeter. Op-amps have two primary properties:
\begin{itemize}
\item{the inputs draw no current} \vspace{-10pt}
\item{the voltages at the inputs to the op-amp are the same}
\end{itemize}
The voltage at the input to the op-amp, $V_1$, is determined by
measuring the voltage at the output of the op-amp, $V_2$, and
accounting for the op-amp's gain such that:
\begin{equation}
V_1 = \frac{R_a}{R_a + R_b} V_2.
\end{equation}
The photo current is therefore $i_\gamma = V_1/R_t$.
...
\section{WFS Control Filters}
\label{sec:WFSfilters}
Figure~\ref{fig:WFSfilters} is a record of the digitial filters used
for the WFS control servo during Enhanced LIGO. 
\begin{figure}
\begin{centering}
\subfigure{\includegraphics[width=1.0\textwidth]{figures/WFSfilters.pdf}}
\caption{WFS digital control filters.}
\label{fig:WFSfilters}
\end{centering}
\end{figure}
...
\caption[]{Ground motion at time of optical lever spectra when the
  interferometer was unlocked (Figure~\ref{fig:seismicMirror}) and at
  time of ASC suppression demonstration (Figure~\ref{fig:ASConoff}). GPS
...
\caption[]{Ground motion at time of ASC suppression demonstration
  (Figure~\ref{fig:ASConoff}). GPS time is 956751915 (May 1, 2010 08:15
  CDT).}
...
\caption[]{Ground motion at time of input beam motion impression
  measurement (Figure~\ref{fig:inputbeam_impression}). GPS time is
...
\caption[]{Ground motion at time of beam spot motion measurement
  (Figure~\ref{fig:bsm}). GPS time is 956728935 (May 1, 2010 01:02
  CDT).}
...
\caption[]{Ground motion at time of ASC to DARM noisebudget plot
  (Figure~\ref{fig:asc2darm}). GPS time is 958456964 (May 21, 2010 01:02
  CDT).}
