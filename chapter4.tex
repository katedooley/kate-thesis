...
% Since the spring constant directly informs the resonant frequency of
% the eigenmode, $\omega_0 = \sqrt{\kappa_{tot}/I}$, it is interesting
% to note that as power increases, the hard mode's resonant frequency
% increases, but the soft mode's resonant frequency decreases. Above the
% critical power for the soft mode, $\omega_0$ becomes imaginary; the
% system has no real resonance. This effect and the splitting of the
% modes is demonstrated in Fig. \ref{fig:TFrainbow}, a plot of the
% torque to angle transfer function for a sampling of spring
% constants. 
...
% The phase of the transfer function is best understood by evaluating
% the poles of the transfer function,
% \begin{equation}
% s = s_\pm = \frac{-\gamma \pm \sqrt{\gamma^2 - 4 I \kappa_{tot}}}{2 I},
% \label{eq:TFpoles}
% \end{equation}
% which indicate whether a system will respond to an excitation in a
% sinusoidal and/or exponential matter\footnote{The inverse Laplace
%   transform of the transfer function is the impulse response.}. As
% long as the poles are negative, the angular motion will decay or be
% sinusoidal. However, if a pole is positive, the system's motion will
% experience exponential growth. When the two poles are negative, each
% contributes $-90^\circ$ of phase to the transfer function, yielding
% the $180^\circ$ drop as seen in Fig. \ref{fig:TFrainbow}.

% The conditions on $\kappa_{tot}$ for $s_\pm$ to be in a particular
% half of the s-plane are easily derived from
% Eq. \ref{eq:TFpoles}. First, we notice that $s_-$ will always be in
% the left half of the plane and that $s_+$ is the potentially
% problematic pole. All possibilities for the pole in question are
% enumerated in Table \ref{table:s_0} and demonstrate that as is shown
% in the previous subsection, stability is determined by the sign of
% $\kappa$. Figure \ref{fig:pendTF_k} shows how the pole locations
% change as a function of $\kappa$, using the same set of values as in
% Fig. \ref{fig:TFrainbow}.



% \begin{table}
% \centering
% \caption{Conditions on total torsional constant $\kappa$ for determining
%   system stability.}
% \begin{tabular}{l l l}
...
% \begin{figure}
% \begin{centering}
% \includegraphics[width=0.8\textwidth]{figures/TFrainbow_poles.pdf}
% \caption{Poles of the torque to pitch transfer function as a function
