...
\textcolor{blue}{Make this section correct!}
...
\section{Design of the Angular Control System}
...
\item wavefront sensors\footnote{Pronounced ``woofs''.} (WFS1, WFS2, WFS3, WFS4)
\end{itemize}
...
\begin{figure} 
\begin{centering} 
\includegraphics{figures/pendulumLoop_control.pdf} 
\caption[]{}
\label{fig:}
\end{centering}
\end{figure}
...
Figure \ref{fig:} shows the simplifed control loop picture of the WFS controlling the optical-lever-controlled mirrors. The torque to angle transfer function of the pendulum is our plant, $P$, the optical lever control is $O$, and the WFS control is $W$. Since the wavefront sensors only indirectly observe the mirror angle, a lot of details are hidden behind $W$. The details are not necessary here, but they are introduced in Sec.~\ref{sec:} and studied in Chapter \ref{ch:characterization}. 

A non-necessary, but useful technique is to
